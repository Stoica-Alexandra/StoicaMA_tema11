...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e) {
            ...
            light1 = new LightSource(LightName.Light1, new float[] { 5.0f, 0.0f, 5.0f, 1.0f });
            //light1.Position = new float[] { 10.0f, 10.0f, 10.0f, 10.0f };
            light1.SetIntenseColor(0.5f, 0.2f, 0.0f, 15.0f);  // Portocaliu foarte intens
            light1.Update();
            light1.Enable();

            // Asociază evenimentul KeyDown
        ...
        private void GlControl1_MouseMove(object sender, MouseEventArgs e) {
            if (statusMouseDown == true) {
                // Sursa de iluminare 2
                float deltaX = (e.X - mousePos.X) * 0.1f;
                float deltaY = (mousePos.Y - e.Y) * 0.1f;
                light1.Move(deltaX, deltaY, 0);  // Modifică poziția luminii
                light1.Update();
        ...
        {
            if (lightON)
            { // Iluminarea globală trebuie să fie activă

                if (light1.IsEnabled)
                {
                    light1.Disable();
                    btnlight1.Text = "Light 1 OFF";
                }
                else
                {
                    light1.Enable();
                    btnlight1.Text = "Light 1 ON";
                }
                GlControl1.Invalidate();
            }
        }
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e) {
            if (e.KeyCode == Keys.W) light1.Move(0, 1, 0);  // Axa Y sus
            if (e.KeyCode == Keys.S) light1.Move(0, -1, 0); // Axa Y jos
            if (e.KeyCode == Keys.A) light1.Move(-1, 0, 0); // Axa X stânga
            if (e.KeyCode == Keys.D) light1.Move(1, 0, 0);  // Axa X dreapta
            if (e.KeyCode == Keys.Q) light1.Move(0, 0, 1);  // Axa Z sus
            if (e.KeyCode == Keys.E) light1.Move(0, 0, -1); // Axa Z jos
            light1.Update();
            GlControl1.Invalidate();
        }
